...
이 글은 **React Router v7의 middleware와 loader 기능을 SPA 환경에서 어떻게 이해하고 사용해야 하는지**에 대하여 작성되어 있고,
모든 내용은 아래 환경을 기준으로 테스트하고 정리했습니다.
...
> SSR, server middleware, Remix 환경은 다루지 않습니다.

따라서 이 글에서 말하는 middleware와 loader는 **모두 브라우저에서 실행되는 클라이언트 로직**입니다.
...
React Router v7의 middleware는 **네비게이션 과정 중, loader나 action이 실행되기 전에 개입할 수 있는 라우터 레벨 로직**입니다.
Express 미들웨어와 달리 HTTP 요청을 직접 처리하지 않고, **라우팅 흐름을 제어하는 역할**을 가집니다.

이전 react router v6 사용 시 화면 진입전에 특정 로직을 넣으려면 HOC 컴포넌트를 만들어서 처리해야 했지만,
v7부터는 라우트 설정에 middleware를 사용하여 페이지가 렌더링 이전에 좀 더 앞단에서 제어가 가능해졌습니다.

SPA 환경에서는 주로 다음 용도로 사용됩니다.
...
middleware는 **라우터가 관리하는 상태로 동작하며, React 컴포넌트 렌더링과는 별개로 실행됩니다**.



### 1-2. loader란 무엇인가

loader는 **라우트 진입 시 필요한 데이터를 준비하는 함수**입니다.
컴포넌트가 렌더되기 전에 실행되며, React Router가 관리하는 상태로 저장됩니다.

loader가 없을 때, 해당 화면에서 필요한 초기 데이터를 useEffect 마운트 시점에 데이터를 패칭하여 가져와서 사용했었습니다.

이로 인해 컴포넌트 플리커링 현상이 있었습니다. 예를 들면 로그인 여부에 따라서 다른 UI를 보여줘야 하는데,
useEffect에서 데이터를 패칭하는 동안은 기본 UI가 잠시 보였다가 데이터가 도착한 후에야 올바른 UI로 바뀌는 현상입니다.
(이 현상을 막는 방법은 미리 페칭이 끝날때까지 렌더링을 하지 않는 꼼수는 있지만 복잡도가 올라가고 유지보수가 어려워집니다.)

Router 구조가 부모 -> 자식 -> 자식2 구조로 각 route 마다 loader를 정의했다면 마지막 자식2 페이지가 렌더링되고 useRouteLoaderData() 훅을
통해서 데이터를 가져올 수 있습니다.

loader를 사용하면

* 컴포넌트 내부 `useEffect`에서 하던 데이터 패칭을 라우터 영역(middleware)으로 옮길 수 있고
* 페이지 단위로 데이터 흐름을 명확하게 정의할 수 있어 라우터 영역에 대한 flow 처리를 하면 더 쉽게 관리할 수 있습니다.
...
아래 예시는 "인증 정보(auth)를 context에 넣어두고, 특정 페이지 loader에서 로그인 여부를 검사"하는 흐름입니다.
...
위 코드를 요약정리하면 다음과 같습니다.

router에 middleware, loader를 등록하고 authMiddleware에서 로그인 체크여부 API를 호출하고 응답 값을 context에 set 처리를 하고 loader에서 context get으로 가져와서 화면으로 내려줍니다.

화면까지 내려가지 않고 인증이 실패하면 middleware에서 `throw redirect(`/login?returnUrl=${encodeURIComponent(new URL(args.request.url).pathname)}`);`
throw redirect를 통해 URL을 이동시킬 수 있습니다.
...
라우터 영역은 **React Router가 실행되는 영역**입니다.
이 영역에서는 UI 렌더링이 아니라, **네비게이션과 데이터 흐름**을 관리합니다.
...
이 단계에서는 아직 React 컴포넌트는 렌더되지 않습니다.



### 2-2. 리액트 영역이란 무엇인가

리액트 영역은 **React 컴포넌트 트리가 실제로 렌더링되는 영역**입니다.
...
중요한 점은, **리액트 영역은 라우터 영역의 결과를 기반으로 렌더링만 수행한다**는 것입니다.

---

## 3. 전체 실행 흐름 한 눈에 보기

SPA 환경에서 네비게이션이 발생했을 때의 전체 흐름은 다음과 같습니다.
...
부모/자식 route 모두 middleware를 가지고 있다면 실행 순서는 다음과 같습니다.

* root → parent → child 순서로 실행
* child route에서 `next()`를 호출하면 loader 실행이 트리거됩니다
* 이후 실행 흐름은 child → parent → root 순으로 되돌아옵니다

middleware 중간에 redirect나 error가 발생하면, 이후 단계는 실행되지 않습니다.



### 4-2. loader 실행 시점

loader는 **leaf route에서 `next()`가 호출되는 순간 실행**됩니다.
이때 실행 대상은 "이번 네비게이션에 필요한 loader list"입니다. 즉, 부모에 있는 loader도 실행됩니다.
...
라우터 영역에서는 다음 작업들이 완료됩니다.

* middleware / loader 실행
* loaderData 및 context 저장
* navigation 상태 확정

이 시점에서 **어떤 화면을 렌더할지에 대한 모든 정보가 준비**됩니다.



### 5-2. 리액트 영역에서 시작되는 일들

라우터 상태가 확정되면 React 렌더링이 시작됩니다.

* route에 매칭된 element 렌더
* 해당 element가 `React.lazy`라면 JS 청크 import 시작
* 로딩 중에는 `<Suspense fallback>`이 먼저 보일 수 있음

즉, **데이터는 준비됐지만 화면 JS가 아직 로딩 중인 상태**가 가능합니다.



### 5-3. loader/context 데이터는 어떻게 사용되는가

loader에서 반환한 데이터는 다음 훅으로 접근합니다.

* `useLoaderData`
* `useRouteLoaderData`

middleware에서 저장한 context 데이터 역시, 이후 loader나 컴포넌트에서 접근 가능합니다.
이 데이터들은 모두 **라우터가 관리하는 상태**입니다.

useLoaderData() 훅은 router 페이지에 연결된 loader context 데이터를 가져와 사용할 수 있습니다.
useRouteLoaderData(:id) 훅은 router id 별로 정의한 loader context 데이터를 사용할 수 있습니다.
...
아래 순서도를 보면 동작 흐름에 대하여 좀 더 명확히 이해할 수 있습니다.
...
middleware는 **"이 라우트에 들어가도 되는가?"**를 판단할 때 적합합니다.

* 로그인 여부 확인
* 권한 없는 페이지 차단
* loader 실행 이전에 빠른 redirect

loader는 **"이 화면을 렌더하기 위해 필요한 데이터"**를 준비할 때 적합합니다.

* 페이지 진입 필수 데이터
* 여러 컴포넌트에서 공유되는 데이터

두 기능을 섞어 쓰기보다는, **책임을 명확히 분리하는 것이 유지보수에 유리합니다.**

예를 들어 Root 영역에서 체크해야하는 middleware (로그인 체크, 페이지 정보...)
실제 렌더링 되는 router에서 실행되는 middleware (초기 상태 데이터 리스트)

middleware를 여러개 공통으로 만들어 화면별로 조합으로 사용하다보니 유지 보수도 깔끔한 장점이 있습니다.
...
SPA 환경에서 middleware는 어디까지나 **클라이언트 가드**입니다.
실제 보안 검증은 반드시 API 서버에서 수행되어야 합니다.



### 7-2. "렌더랑 동시에 실행된다"는 착각

middleware와 loader는 **렌더 전에 실행**됩니다.
또 운영상 배포를 주기적으로 하다보면 청크파일이 변경되면서 에러 페이지가 노출되는 케이스가 있는데
middleware, loader를 통하면 청크 JS를 로드하기 전에 청크파일 변경 여부를 알 수도 있습니다.

청크가 변경되었다면 리액트 렌더링 단계 진입 전에 강제로 새로고침을 유도하는 로직을 middleware에 넣을 수 있습니다.


### 7-3. middleware 로직이 과하면 안된다.

Middleware 영역에서 무거운 API를 호출하거나 복잡한 비즈니스 로직을 실행할 경우, 그만큼 사용자에게 흰 화면이 노출되는 시간이 길어질 수 있으므로 주의가 필요합니다.

앱에서 사용하는 초기 레이아웃 데이터, 인증·검증·인가 로직 등을 무분별하게 middleware 영역으로 이동하기보다는, 반드시 영향도 분석과 함께 실제로 더 효율적인 선택인지 충분히 고민한 후 작업해야 합니다.

API 응답이 지연되는 상황은 언제든 발생할 수 있으므로,
타임아웃을 설정해서 오랫동안 대기하지 않도록 하고, 일정 시간 초과 시 즉시 화면에 진입시키거나 Error Boundary로 이동시키는 로직은 고민하여 추가해야 할 것 같습니다.
...
React Router v7의 middleware와 loader는 필수 기능은 아닙니다.
하지만 **라우터가 데이터 흐름을 주도하도록 설계하면**, 컴포넌트는 훨씬 단순해집니다.

이 기능들의 핵심은 "편의성"이 아니라 **실행 흐름을 명확하게 만드는 것**입니다.
